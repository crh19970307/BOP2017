...
def output():
	...
	model=load_model('model/model/sentencelen35_230000_lstm128_epoch010_model.h5')

	quelist,answerlist,datalist=getdata()
	del quelist,answerlist 

	wordlist=spiltword(datalist)
	del datalist 
	vectorlist=turnwordtovector(wordlist)
	del wordlist 
	f=open('result/score_sentencelen35_230000_lstm128_epoch010_model.txt','w',encoding='utf-8')

	xa=np.zeros((len(vectorlist),35,60),dtype='float64')
...
def test():
	...
	model=load_model('model/model/sentencelen30_230000_lstm128_epoch010_model.h5')
	...
	score=model.predict(xa)

	for index,item in enumerate(score):

		score[index][0]=float('%.6f'%(score[index][0]))
...
#	print('\n\n\n\n\n')
	...
	#print('The score in file is ')

	#print(float(str(score[0:30][0])))	\

#	for i in range(300):

#		print(float(str(score[i][0])))
...
#	exit(0)
	...
	for cnt,question in enumerate(quelist):
		...
		if answerlist[cnt]==[]:
			...
			sum=sum+1/(1+tmp.index(score[answerlist[cnt]][0]));
