...
def savetojson():
	...
	quelist,answerlist,datalist=getdata('train.txt')
...
def train():
	...
	quelist,answerlist,datalist=getdata('dev.txt')
	wordlist1,wordlist2=spiltword(datalist)
	vectorlist1,vectorlist2=turnwordtovector(wordlist1,wordlist2)
	f=open('auto_finetune_log2.txt','w',encoding='utf-8')
	for quelen in [10,15,20]:
		for anslen in [15,20,25]:
			for epoch in [7,9,12,15]:
				xa=np.zeros((len(data['vectorlist1']),quelen+anslen,60),dtype='float64')
				...
				for index1,items in enumerate(data['vectorlist2']):
					for index2,item2 in enumerate(items):
						if index2==anslen:
							break
						xa[index1][index2+quelen]=item2	
				xb=np.zeros((len(vectorlist1),quelen+anslen,60),dtype='float64')
				for index1,items in enumerate(vectorlist1):
					for index2,item2 in enumerate(items):
						if index2==quelen:
							break
						xb[index1][index2]=item2
				for index1,items in enumerate(vectorlist2):
					for index2,item2 in enumerate(items):
						if index2==anslen:
							break
						xb[index1][index2+quelen]=item2	
				...
				model.add(LSTM(128,input_length=quelen+anslen,input_dim=60))
				print('LSTM added')
				model.add(Dropout(0.5))  	
				model.add(Dense( 1))  
				model.add(Activation('sigmoid'))  
				model.compile(loss='binary_crossentropy', optimizer='adam', class_mode="binary")  
				model.fit(xa, ya, batch_size=128, nb_epoch=epoch) #训练时间为若干个小时  
				model.save('sentencelen'+'%02d+%02d'%(quelen,anslen)+'_230000_lstm128_epoch'+'%03d'%(epoch)+'_model.h5')
				score=model.predict(xb)
				#score=[]
				sum=0
				...
				print('\n\n\n\n\n')
				print('quelen'+str(quelen)+' anslen=' +str(anslen)+' epoch='+str(epoch))
				print('MRR='+str(sum/len(quelist)))
				f.write('quelen'+str(quelen)+' anslen=' +str(anslen)+' epoch='+str(epoch))	
				f.write('MRR='+str(sum/len(quelist)))
				f.write('\n')
	f.close()
