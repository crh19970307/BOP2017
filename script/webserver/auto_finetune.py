...
def train():
	...
	f=open('auto_finetune_log.txt','w',encoding='utf-8')
	for sentencelen in [30,35]:
		for epoch in [8,9,11,13]:
			xa=np.zeros((len(data['vectorlist']),sentencelen,60),dtype='float64')
			for index1,items in enumerate(data['vectorlist']):
				for index2,item2 in enumerate(items):
					if index2==sentencelen:
						break
					xa[index1][index2]=item2
			xb=np.zeros((len(vectorlist),sentencelen,60),dtype='float64')
			...
			model.add(LSTM(128,input_length=sentencelen,input_dim=60))
			print('LSTM added')
			...
			model.save('model/sentencelen'+'%02d'%(sentencelen)+'_230000_lstm128_epoch'+'%03d'%(epoch)+'_model.h5')
			...
			for cnt,question in enumerate(quelist):
				...
				if answerlist[cnt]==[]:
					...
					tmp.reverse()
					#if tmp.indexscore[answerlist[cnt]])!=0:
					#	print(tmp)
					sum=sum+1/(1+tmp.index(score[answerlist[cnt]]));
			print('\n\n\n\n\n')
			print('sentencelen='+str(sentencelen)+'  epoch='+str(epoch))
			print('MRR='+str(sum/len(quelist)))
			f.write('sentencelen='+str(sentencelen)+'  epoch='+str(epoch))	
